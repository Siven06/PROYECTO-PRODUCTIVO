...
<style>
  ...
  .jw-btn-deactivate:hover { background: rgba(245,158,11,0.18); }
  .jw-btn-activate {
    background: rgba(16,185,129,0.1); color: #065f46;
    border: 0.5px solid rgba(16,185,129,0.25);
  }
  .jw-btn-activate:hover { background: rgba(16,185,129,0.18); }
  ...
  /* Avatar + nombre */
  .jw-user-cell { display: flex; align-items: center; gap: 12px; }
  .jw-avatar {
    width: 38px; height: 38px; border-radius: 50%;
    background: var(--gold-dim); border: 1.5px solid var(--gold-border);
    display: flex; align-items: center; justify-content: center;
    font-family: var(--font-display); font-size: 16px;
    color: var(--gold); flex-shrink: 0; font-weight: 600;
  }
  .jw-user-name { font-size: 13px; font-weight: 500; color: #1a1a1a; }
  .jw-user-id   { font-size: 11px; color: #9ca3af; }
  ...
  .jw-email i { font-size: 11px; color: #9ca3af; }

  /* Badges de rol */
  .jw-role-badge {
    display: inline-block; font-size: 10px; padding: 3px 10px;
    border-radius: 20px; font-weight: 500; margin: 2px;
    ...
    <div class="jw-card">
      ...
      <div class="jw-card-header">
        <h3 class="jw-section-title">
          <i class="bi bi-people me-2" style="color:var(--gold);font-size:18px;"></i>
          Usuarios
        </h3>
        <div class="jw-toolbar">
          <form action="{{ route('usuarios.index') }}" method="get" class="jw-search-form">
            <input name="texto" type="text" value="{{ $texto }}" placeholder="Buscar usuario...">
            <button type="submit">
              <i class="bi bi-search"></i> Buscar
            </button>
          </form>
          @can('user-create')
          <a href="{{ route('usuarios.create') }}" class="jw-btn jw-btn-new">
          ...
          <table class="jw-table">
            ...
            <tbody>
              ...
              @if(count($registros) <= 0)
                <tr class="empty-row">
                  <td colspan="5">
                    <i class="bi bi-people" style="font-size:32px;display:block;margin-bottom:8px;color:var(--gold);opacity:0.4;"></i>
                ...
                <tr>
                  ...
                  <td>
                    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                      @can('user-edit')
                      <a href="{{ route('usuarios.edit', $reg->id) }}" class="jw-btn jw-btn-edit">
                      ...
                      @can('user-activate')
                      <button class="jw-btn {{ $reg->activo ? 'jw-btn-deactivate' : 'jw-btn-activate' }}"
                              data-bs-toggle="modal"
                              data-bs-target="#modal-toggle-{{ $reg->id }}">
                        @if($reg->activo)
                          <i class="bi bi-ban"></i> Desactivar
                        @else
                  ...
                  <td>
                    <div class="jw-user-cell">
                      <div class="jw-avatar">
                        {{ strtoupper(substr($reg->name, 0, 1)) }}
                      </div>
                      <div>
                        <div class="jw-user-name">{{ $reg->name }}</div>
                        <div class="jw-user-id">#{{ $reg->id }}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    @if($reg->roles->isNotEmpty())
                      @foreach($reg->roles as $rol)
                        <span class="jw-role-badge">
                          <i class="bi bi-shield-check" style="font-size:9px;"></i>
                          {{ $rol->name }}
                        </span>
                      @endforeach
                    @else
                      <span class="jw-role-none">Sin rol asignado</span>
                    @endif
                  </td>

                  {{-- Estado --}}
                  <td>
                    <span class="jw-status-badge {{ $reg->activo ? 'status-active' : 'status-inactive' }}">
                      <span class="dot"></span>
